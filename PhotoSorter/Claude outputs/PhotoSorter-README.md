...
### 4. Create a Supabase project (backend for login/signup)

1. Go to [supabase.com](https://supabase.com) and create a free project.
2. In the project dashboard, go to **Settings -> API** and copy:
   - **Project URL**
   - **anon public** key
3. Open [Sources/Services/SupabaseManager.swift](Sources/Services/SupabaseManager.swift)
   and paste them in place of the placeholder values:
...
4. (Optional, for faster testing) In Supabase, go to
   **Authentication -> Providers -> Email** and turn off "Confirm email" so
   new accounts can log in immediately without clicking an email link.
